...
class CLIPAdapter(nn.Module):
    ...
    def __init__(self, clip_model, adapter_ratio=0.2):
        ...
        self.adapter = Adapter(output_dim, 4).to(clip_model.dtype)
        self.ratio = adapter_ratio

    def encode_image(self, image):
        with torch.no_grad():
            features = self.clip.encode_image(image)
        adapted = self.adapter(features.float()).to(features.dtype)
...
class CLIPInferencer:
    ...
    def classify(self, image_path: str, classes: list) -> dict:
        ...
        text_tokens = clip.tokenize(classes).to(self.device)
