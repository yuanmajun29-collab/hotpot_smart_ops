...
from platform.cloud.event_hub.auth import AuthContext, get_auth_context, enforce_admin, data_scope_for_role
from platform.cloud.event_hub.device_stub import get_pipeline_status, run_subprocess_pipeline, tick_all_stores_inprocess, tick_store_inprocess
from platform.cloud.event_hub.routers._deps import AdminStoreCreate, AdminStoreUpdate, PipelineTickBody
...
def admin_list_users(auth: AuthContext = Depends(get_auth_context)) -> Dict[str, Any]:
    enforce_admin(auth)
    from platform.cloud.event_hub.auth import DEMO_USERS
