...
> **运营副驾驶**：先帮门店把后厨损耗看见、算清、预测并闭环，再扩展到翻台、SOP、日报和多店运营。
...
| 痛点 | 产品手段 |
|------|----------|
| 来料短重、损耗难量化、备货不准 | 秤 + VLM + ERP/POS + IoT → 损耗归因与预测 |
...
登录 RBAC · 后厨损耗闭环主路径 · 看板 7 模块 · 收货 PDA 5 步 · 企微 critical · 两店预配置 UAT
...
### 目标 2：降低食材损耗（Phase 1 主切入口）

| 项 | 内容 |
|----|------|
| **产品承载** | IoT 全链路 F-K03 · 成本页 F-C01~C07 · PDA 验收 F-P01~P06 |
| **成功信号** | 来料偏差可见、可追责；短重批次标红；损耗风险 TopN 可处理 |
| **当前** | ⚠️ 成本列表 + 偏差高亮有；IoT/秤/VLM/PDA 多为静态或 mock |
| **差距** | 真 MQTT 传感器未上线；PDA 未接 ERP/秤/VLM/Hub 提交；签字未入库 |
| **下一里程碑** | **DEV-411~413** 真 IoT · **DEV-416~419** PDA 全链路 · **DEV-420** 签字 API · **LOSS-401** 损耗风险 TopN baseline |
| **责任人** | 嵌入式 + 前端 + 后端 |
