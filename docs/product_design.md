...
> **主路径：后厨损耗预测闭环** = ERP/POS/采购单基线 + PDA 收货/称重/签字 + IoT 冷链/门磁 + 成本页归因 + 告警/SOP/日报闭环；P1C 后以 VLM 影子模式补充废弃区/收盘区视觉浪费证据。
...
| 后续扩展 | 视觉浪费证据、翻台效率、SOP 合规、增收推销、供应商 KPI 按阶段逐个突破 |
...
| F-C03 | VLM 品质等级 | P0 | 作为收货员，外观等级要记录 | A/B/C/D + 截图。**创业 MVP 阶段降级为师傅手动 3 按钮打分（好/一般/差）作为损耗预测输入因子，VLM 自动验货延后至 P1B 后/二期** — 见 [损耗预测切入计划 §8.3](kitchen_loss_prediction_wedge_plan.md) |
...
| F-C09 | 视觉浪费证据 | P1C/P2 | 作为店长/厨师长，我想看到废弃区和收盘区真实浪费证据 | `/v1/vlm/waste-estimate` 输出 SKU/状态/浪费类型/估重/置信度/建议动作；P1C 影子模式只出报告，不改采购、不处罚个人 |

**PoC 已有**：F-C01~C02 摘要（cost-summary + cost-list）

**创业切入优先级**：F-C01~C04 + F-P01~P06 + F-K01~K03 是 P1A“损耗可见”主线；F-C06 是 P1B“损耗预测”主线；F-C07 与 F-TASK/SOP 指派衔接，作为 P1C/P1.5 行动闭环；F-C09 是 P1C/P2 的视觉证据补充，不替代称重/温度/PDA 主证据。

### 5.5.1 模块：增收/推销（F-SALES · Phase 2）

| ID | 功能 | 优先级 | 用户故事 | 验收标准 |
|----|------|--------|----------|----------|
| F-SALES01 | 规则版推销建议 | P2 | 作为营销运营，我想按桌态/时段给门店提示 | 规则命中后生成建议，不自动触达顾客 |
| F-SALES02 | 营销话术库 | P2 | 作为营销运营，我想维护标准话术 | 仅 `marketing_ops` 可编辑，PMO 可审计 |
| F-SALES03 | 推销任务下发 | P2 | 作为店长，我想把建议派给班组执行 | 可生成 F-TASK 任务，门店人工确认 |

**边界**：会员画像、会员营销自动化延后 P3；F-SALES 不进入 Phase 1 UAT。
...
### 5.8.1 模块：全链路追溯（F-TRACE · Phase 2）

| ID | 功能 | 优先级 | 用户故事 | 验收标准 |
|----|------|--------|----------|----------|
| F-TRACE01 | 追溯链查询 | P2 | 作为财务/督导，我想从批次追到事件和整改 | 通过 `ref_type/ref_id/trace_id` 串联 OpsEvent、tasks、签字、日报 |
| F-TRACE02 | 任务与事件互链 | P2 | 作为店长，我想从告警看到整改结果 | task 详情可回到原始告警/来料/SOP |

**实现原则**：复用 ADR-002 `OpsEvent` 与 F-TASK `task_events`，不新建大而全追溯表。
...
| 权限 | 店长 | 班组长 | 领班 | 厨师长 | 收货员 | 督导 | 营销运营 | 财务审计 | 总部 | CEO | 加盟业主 |
|------|:----:|:------:|:----:|:------:|:------:|:----:|:--------:|:--------:|:----:|:---:|:--------:|
| 看板只读 | ✓ | ✓ | ✓ | ✓ | — | ✓ | — | 成本/追溯 | ✓ | 摘要 | ✓ |
| 告警 ack | ✓ | ✓ | ✓ | ✓ | — | ✓ | — | — | — | — | — |
| 桌态纠正 | ✓ | ✓ | ✓ | — | — | — | — | — | — | — | — |
...
| 整改指派 | ✓ | ✓ | — | ✓ | — | ✓ | — | — | — | — | — |
| 任务 reassign | ✓ | ✓ | — | — | — | ✓ | — | — | — | — | — |
| 任务 cancel/reopen | ✓ | — | — | — | — | ✓ | — | — | PMO | — | — |
| F-SALES 规则 | — | — | — | — | — | 只读 | ✓ | — | PMO 审计 | — | — |
| 成本/追溯审计 | ✓ | — | — | ✓ | — | ✓ | — | ✓ | ✓ | 摘要 | 本店只读 |
| SOP/阈值配置 | — | — | — | — | — | — | — | — | ✓ | — | — |
| 跨店数据 | — | — | — | — | — | ✓ | 按 scope | 按 scope | ✓ | 全国摘要 | 本店 |
...
**Phase 2 Should Have**：F-SALES01~03（rule-based 推销/增收）、F-TRACE01~02（复用 OpsEvent/task_events 的追溯链）。
