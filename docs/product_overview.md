...
**原则**：驾驶仓 / 层级看板 **只读**；配置写入 **仅运营后台**；门店一线 **不碰全局配置**。设计与落地分期见 [ADR-013](architecture_decisions.md#adr-013设计先行实现与真数据接入分期)。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| V1.1 | 2026-06-16 | 四类产品面 · 8 角色 · 完成度 · 阶段路线 |
| V1.2 | 2026-06-16 | 对齐 ADR-013：终局设计写全、实现与真数据分期 |
