...
| W7 | 全国总揽页 UI | DEV-504 | `national.html`（API `/v1/national/overview` 已有，F-EXEC01 已用） |
| W8 | 审计日志 | DEV-505 | 增删改用户/店留痕 |
| W9~10 | 配置 OTA + F-SALES 规则版 | DEV-506/529 | SOP JSON、阈值 webhook、营销话术规则 |
...
| 任务督办 | `sop_assignments` | `tasks` + `task_events` | DEV-520~524 |
| 推销/增收 | — | `sales_rules` + F-TASK | DEV-529 |
| 追溯 | OpsEvent/签字/日报分散 | `trace_id/ref_id` 串联查询 | DEV-530 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| V1.3 | 2026-06-16 | DEV-529/530 统一编号；F-EXEC01/API 与 F-HQ12/UI 分工 |
